...
def get_poster(query):
    ...
    try:
        ...
        # Search IMDB
        search_url = f"https://www.imdb.com/find?q={quote_plus(query)}&s=tt&ttype=ft,tv"
        headers = {
        ...
        if response.status_code != 200:
            LOGGER.warning(f"IMDB search failed with status code: {response.status_code}")
            return None
        ...
        # Extract year
        year = ""
        year_elem = soup.find('a', {'href': lambda x: x and '/releaseinfo' in x if x else False})
        if year_elem:
            year = year_elem.text.strip()
        
        # Extract rating
        rating = ""
        rating_elem = soup.find('span', {'class': lambda x: x and 'sc-' in x if x else False})
        ...
        # Extract poster
        poster = ""
        poster_elem = soup.find('img', {'class': lambda x: x and 'ipc-image' in x if x else False})
