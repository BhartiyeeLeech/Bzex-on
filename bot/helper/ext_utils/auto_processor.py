...
class AutoProcessor:
    ...
    async def process_auto_message(client, message: Message):
        ...
        try:
            ...
            if auto_yt_leech and has_video_url:
                ...
                LOGGER.info(
                    f"Auto processing triggered for user {user_id}: Media file detected"
                )
                ...
                is_leech = auto_leech  # AUTO_LEECH has priority
                LOGGER.info(
                    f"Auto processing triggered for user {user_id}: URL detected"
                )
            ...
            LOGGER.error(
                f"Auto processing failed for user {user_id}: {e}", exc_info=True
            )
        finally:
    ...
    async def _process_media(client, message: Message, is_leech: bool):
        ...
        if (
            not hasattr(command_message, "_client")
            or command_message._client is None
        ):
        ...
        # Create Mirror task
        LOGGER.info(
            f"Creating Mirror task for media file from user {message.from_user.id}"
        )
        mirror_task = Mirror(
    ...
    async def _process_url(
        ...
        if is_video_url:
            command_name = (
                ...
                if is_leech
                else BotCommands.YtdlCommand[0]
            )
        else:
            command_name = (
                BotCommands.LeechCommand[0]
                if is_leech
                else BotCommands.MirrorCommand[0]
            )
        ...
        if is_video_url:
            LOGGER.info(
                f"Creating YtDlp task for video URL from user {message.from_user.id}"
            )
            ytdlp_task = YtDlp(client, command_message, is_leech=is_leech)
            await ytdlp_task.new_event()
        else:
            LOGGER.info(
                f"Creating Mirror task for URL from user {message.from_user.id}"
            )
            mirror_task = Mirror(
                client, command_message, is_qbit=False, is_leech=is_leech
            )
...
def auto_message_filter(_, __, message: Message) -> bool:
    ...
    if auto_yt_leech and not (auto_leech or auto_mirror):
        ...
        if has_url:
            ...
            for word in message_text.split():
                if is_url(word) and any(
                    domain in word.lower() for domain in video_domains
                ):
                    return True
        return False
